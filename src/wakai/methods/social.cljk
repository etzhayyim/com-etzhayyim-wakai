(ns wakai.methods.social
  "social.cljc — 和会 — Mutual Aid DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from wakai's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "和会 — Mutual Aid が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over wakai's own domain (Non-profit religious-corp mutual aid substrate. NOT insurance (no premium-as-contract; no actuarial pricing; no claim adjudication; no…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "wakai R0: live social posting is Council Lv6+ + operator + "
